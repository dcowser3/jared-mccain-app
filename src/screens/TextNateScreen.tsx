...
const FALLBACK_RESPONSES = [
  'bruh 💀💀💀',
  'no cap that\'s actually insane lmfaooo',
  'bro said that with his whole chest 😭',
  'L take + ratio + you fell off',
  'nah this is crazy LMAOOO 😂😂',
...
export default function TextNateScreen() {
  const [messages, setMessages] = useState<Message[]>([
    {
      id: '1',
      text: 'yooo 💀 what do u want lmaooo',
      sent: false,
      timestamp: new Date(),
    },
  ]);
  ...
  const sendMessage = useCallback(async () => {
    ...
    try {
      const response = await chatApi.sendToNate(trimmed);
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        <Text style={styles.headerTitle}>Nate 🎮</Text>
      </View>
      <KeyboardAvoidingView
        style={styles.flex}
